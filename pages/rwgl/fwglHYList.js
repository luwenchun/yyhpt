...
var sDjzt = "0";            //登记状态（记录服务登记块状数字）
var sPjzt;            //评价状态（记录服务评价块状数字）
var sHfzt;            //回访状态（记录服务回访块状数字）
        ...
        .bootstrapTable(
            {
                ...
                url: "yyhptRwglFwglhy.do?action=list_yh", // 获取数据的Servlet地址
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            if (value == '0') {
                                return '<a class="fwlbToFwpj" href="javascript:void(0)"><img src="layouts/img/table/icon_del.png">医护评价</a>';
                            } else {
...
FwpjFunc = function (row, mtitle, mflag) {
    ...
    BootstrapDialog.show({
        ...
        closeByBackdrop: false,
        closeByKeyboard: false,
        message: $('<div></div>').load('yyhptRwglFwglhy.do?action=yhpjInit'),
        buttons: [{
            label: '保存',
            id: 'btn_Save',
...
function getFwsc() {
    $.ajax({
        url: "yyhptRwglFwglhy.do?action=fwpjFwxmmx",
        type: "post",
        dataType: "json",
...
FwhfFunc = function (row, mtitle, mflag) {
    ...
    BootstrapDialog.show({
        ...
        closeByBackdrop: false,
        closeByKeyboard: false,
        message: $('<div></div>').load('yyhptRwglFwglhy.do?action=fwhfInit'),
        buttons: [{
            label: '保存',
            cssClass: 'btn-default btn-sm',
            id: 'btn_Save',
...
$(function () {
    ...
    $('#btn_query').on('click', function () {
        $('.counter').parent().removeClass('block-select');
        sDjzt = "";
        sPjzt = "";
        sHfzt = "";
    ...
    //---------------------------------绑定数字事件begin---------------------------------
    $('#fwdjWdjNum').parent().on('click', function () {
        $('.counter').parent().removeClass('block-select');
    ...
    $('#fwdjYdjNum').parent().on('click', function () {
        $('.counter').parent().removeClass('block-select');
        $(this).addClass('block-select');
        InitSearch();
        ztSelect2Change('3');
    ...
    $('#fwpjWpjNum').parent().on('click', function () {
        $('.counter').parent().removeClass('block-select');
        $(this).addClass('block-select');
        InitSearch();
        ztSelect2Change('4');
    ...
    $('#fwpjYpjNum').parent().on('click', function () {
        $('.counter').parent().removeClass('block-select');
        $(this).addClass('block-select');
        InitSearch();
        ztSelect2Change('5');
    ...
    $('#fwhfWhfNum').parent().on('click', function () {
        $('.counter').parent().removeClass('block-select');
        $(this).addClass('block-select');
        InitSearch();
        ztSelect2Change('6');
    ...
    $('#fwhfYhfNum').parent().on('click', function () {
        $('.counter').parent().removeClass('block-select');
        $(this).addClass('block-select');
        InitSearch();
        ztSelect2Change('7');
    ...
    var ztArr = [{'id': '1', 'text': '全部'}, {'id': '2', 'text': '待登记'},
        {'id': '3', 'text': '已登记'}, {'id': '4', 'text': '待评价'},
        {'id': '5', 'text': '已评价'}, {'id': '6', 'text': '待回访'},
        {'id': '7', 'text': '已回访'}]
    ...
    $('#zt').select2({
        ...
        if ($('#zt').val()) {
            if ($('#zt').val()[0] == '1') {
                $('#zt').val(["2", "3", "4", "5","6","7"]).trigger("change");
            }
        }
    });
});
...
function GetRqNum() {
    $.ajax({
        url: 'yyhptRwglFwglhy.do?action=numYh',
        type: "get",
        dataType: "json",
        data: {},
        success: function (data) {
            displayNumberOfBlock($('#fwdjBlock'), $('#fwdjWdjNum'), $('#fwdjYdjNum'), data.WDJ, data.YDJ);
            displayNumberOfBlock($('#fwpjBlock'), $('#fwpjWpjNum'), $('#fwpjYpjNum'), data.WPJ, data.YPJ);
            displayNumberOfBlock($('#fwhfBlock'), $('#fwhfWhfNum'), $('#fwhfYhfNum'), data.WHF, data.YHF);
