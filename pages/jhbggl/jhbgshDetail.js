...
function setGrForm(datas, row) {
    jalx = row.JALX;
    xt_jlzt = row.XT_JLZT;
    $('#XM').text((datas.grjbxx)[0].XM ? (datas.grjbxx)[0].XM : '');
    $('#XBMC').text((datas.grjbxx)[0].XB ? (datas.grjbxx)[0].XB : '');
    $('#CSRQ').text(jsGetAge((datas.grjbxx)[0].CSRQ) ? jsGetAge((datas.grjbxx)[0].CSRQ) : '');
    $('#YLFYZFFSMC').text((datas.grjbxx)[0].YLFYZFFSMC ? (datas.grjbxx)[0].YLFYZFFSMC : '');
    $('#LXDH').text((datas.grjbxx)[0].LXDH ? (datas.grjbxx)[0].LXDH : '');
    $('#LXDZ').text((datas.grjbxx)[0].JZDZ ? (datas.grjbxx)[0].JZDZ : '');
...
function getRylist1() {
    $.ajax({
        ...
        success: function (data) {
            ...
            console.log('rybm:' + rybm);$('#shrgh').val(rybm).trigger("change");
           // $('#shrq').prop('disabled', true);
        ...
        form1.validate({
            ...
            submitHandler: function (form) {
                ...
                if (jalx == '1') {
                    ...
                    var Url = "yyhptjhbggl.do?action=jhshUpadateJhbgsq";
                    $.ajax({
                        url: Url,
                        type: "post",
                        dataType: "json",
