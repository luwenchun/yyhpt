			...
			.bootstrapTable(
					{
						...
						url : "yyhptjcwhzxsj.do?action=list", // 获取数据的Servlet地址
						contentType : "json",
						iconSize : 'sm',
						showHeader : true,
						striped : true, // 表格显示条纹
...
modelFunc = function(row, mtitle, mflag) {
	...
	BootstrapDialog.show({
		...
		message : $('<div></div>').load('yyhptjcwhzxsj.do?action=add'),
		buttons : [ {
			label : '保存',
	...
	BootstrapDialog.confirm({
		...
		callback : function(result) {
			...
			if (result) {
				...
				$.ajax({
					url : "yyhptjcwhzxsj.do?action=delete",
					type : "get",
					dataType : "json",
					data : {
						LSH : rows
