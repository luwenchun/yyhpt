...
// 初始化表格,动态从服务器加载数据
    $("#table_fwjhzx").bootstrapTable({
        ...
        url: "yyhptjhgl.do?action=jhzxList", // 获取数据的Servlet地址
    ...
    BootstrapDialog.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                $.ajax({
                    url: "yyhptjhgl.do?action=deleteJhzxFwxm",
                    type: "post",
                    dataType: "json",
                    data: {
                        jhmxlshs: jhmxlshs
...
runData = function (row, mTitle, mFlag) {
    console.log("jhlxType=");console.log(jhlxType);
    BootstrapDialog.confirm({
        title: '提示信息',
        message: mTitle,
        type: BootstrapDialog.TYPE_WARNING,
...
showRwxxModelFunc = function (mtitle) {
    ...
    console.log('dialogData='); console.log(dialogData.JHLSH);console.log(dialogData);console.log(dialog_data);
    BootstrapDialog.show({
        title: mtitle,
        size: BootstrapDialog.SIZE_WIDE,
        closable: true,
    ...
    BootstrapDialog.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                var rows = allData;
                $.ajax({
                    url: "yyhptjhgl.do?action=jhzxStop",
                    type: "post",
                    dataType: "json",
                    data: {
                        rows: JSON.stringify(rows)
