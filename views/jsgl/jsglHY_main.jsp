...
<div class="panel-body">
    <form id="id_jsgl_list_form" method="post" class="form-horizontal">
        <div id="rqlbDiv" class="panel panel-default">
            ...
            <div class="col-md-1 pull-right" style="margin-bottom: 15px;">
...
<script type="text/javascript">
    ...
    function calcOnCheck(row) {
        ...
        wholeVar.ysje += parseFloat(row.HJJE);
    ...
    function gainAccountPer(queryParams) {
        $.ajax({
            url: 'yyhptjsglHy.do?action=per_list',
            type: 'post',
            dataType: 'json',
            data: queryParams,
            success: function (data) {
                if (data != null || data != undefined) {
                    if (data.xzrylb.length == 1) {          //if 待结算人员只有一个就直接显示其结算记录信息
                        ...
                        getJsglList(params);
                    } else if (data.xzrylb.length > 1) {                                //else 待结算人员有多个，显示带选择人员列表供选择
    ...
    function getJsglList(queryParams) {
        $.ajax({
            url: 'yyhptjsglHy.do?action=list',
            type: 'post',
            dataType: 'json',
            data: queryParams,
            success: function (data) {
    ...
    function chooseDetail(row, title, value) {
        BootstrapDialog.show({
            ...
            message: $('<div></div>').load('yyhpt/views/jsgl/choosePerHy.jsp'),
            buttons: [{
                label: '确认',
    ...
    function doAccount() {
        ...
        BootstrapDialog.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.ajax({
                        url: "yyhptjsglHy.do?action=saveDatasHY", //getSysCzrylist改为获取医护人员中的护士（及本人）
                        type: "post",
                        dataType: "json",
                        data: {
    ...
    function getPara(params) {
        //已结算、未结算都勾选或都未勾选都查询全部数据，只勾选一个则查询未结算或者已结算数据
        if ($('#wjsCk').prop("checked") && $('#yjsCk').prop("checked")) {
            wholeVar.sJszt = '2';
        } else if (!$('#wjsCk').prop("checked") && !$('#yjsCk').prop("checked")) {
            wholeVar.sJszt = '2';
        } else {
            //勾选未结算
            if ($('#wjsCk').prop("checked")) {
                wholeVar.sJszt = '0';
            }

            //勾选已结算
            if ($('#yjsCk').prop("checked")) {
                wholeVar.sJszt = '1';
            }
        }
    }
