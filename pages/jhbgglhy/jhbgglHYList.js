        ...
        onhidden: function (dialogRef) {
            ...
            if (selects.length > 0) {
                BootstrapDialog.show({
                    ...
                    closeByBackdrop: false,
                    closeByKeyboard: false,
                    message: $('<div></div>').load('yyhpt/views/jhbgglhy/jhbgsqDetail.jsp'),
                    buttons: [{
                        id: 'id_apply_save',
                        label: '保存',
                        cssClass: 'btn-default btn-sm',
...
function initTable() {
    ...
    $('#table').bootstrapTable({
        ...
        url: "yyhptjhbgglhy.do?action=getJhbgLbInfo", // 获取数据的Servlet地址
    ...
    if (row.SHZT == '1') {
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action: function (dialog) {
                    $.ajax({
                        url: 'yyhptjhbgglhy.do?action=deletejhbgsq',
                        type: 'get',
                        dataType: 'json',
                        data: {
                            yngrbsh: row.YNGRBSH,
...
function loadJhbgshDetail(row, title, value) {
    if (row.BGSQLSH == '' || row.BGSQLSH.length == 0) {
        ...
        BootstrapDialog.show({
            ...
            closeByBackdrop: false,
            closeByKeyboard: false,
            message: $('<div></div>').load('yyhpt/views/jhbgglhy/jhbgshDetail.jsp'),
            buttons: [{
                label: '保存',
                cssClass: 'btn-default btn-sm',
                id: 'btn_jhbgsh_save',
...
function loadJhbgjlDetail(row, title, value) {
    if (row.BGSQLSH == '' || row.BGSQLSH.length == 0) {
        ...
        BootstrapDialog.show({
            ...
            closeByBackdrop: false,
            closeByKeyboard: false,
            message: $('<div></div>').load('yyhpt/views/jhbgglhy/jhbgjlHYDetail.jsp'),
            onshow: function (dialogRef) {
            },
            onshown: function (dialogRef) {
                dialoging = dialogRef;
...
function loadJhbgjlshDetail(row, title, value) {
    if (row.BGJLLSH == '' || row.BGJLLSH.length == 0) {
        ...
        BootstrapDialog.show({
            ...
            closeByBackdrop: false,
            closeByKeyboard: false,
            message: $('<div></div>').load('yyhpt/views/jhbgglhy/jhbgjlshHYDetail.jsp'),
            onshow: function (dialogRef) {
            },
            onshown: function (dialogRef) {
...
function loadJhbgjlzxDetail(row, title, value) {
    if (row.XJHLSH == '' || row.XJHLSH == null) {
        ...
        BootstrapDialog.show({
            ...
            closeByBackdrop: false,
            closeByKeyboard: false,
            message: $('<div></div>').load('yyhpt/views/jhbgglhy/jhbgjlzxDetail.jsp'),
            onshow: function (dialogRef) {
            },
            onshown: function (dialogRef) {
...
function initHcTable() {
    ...
    $.each(obj.xmhcInfo, function (k, v) {
        var xmhcObj = {HCDM: "", HCDW: "", GG: "", HCMC: '', SL: '', DJ: '', JLLXDM: '', HCSL: '', XZGG: '', HCGG: ''};
        xmhcObj.HCDM = v.HCDM;
        xmhcObj.HCMC = v.HCMC;
        xmhcObj.HCDW = v.HCDW;
        xmhcObj.GG = v.GG;
...
function setHcTable() {
    $.each(xmhcrows, function (k, v) {
        ...
        if (v.XZGG != null) {
            var xzggArray = v.XZGG.toUpperCase().split(',');
